...
std::vector<Triangle> load_obj() {
  ...
  for (size_t s = 0; s < shapes.size(); s++) {
    ...
    for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
      ...
      tinyobj::index_t idx_1 = shapes[s].mesh.indices[index_offset + 0];
      tinyobj::index_t idx_2 = shapes[s].mesh.indices[index_offset + 1];
      tinyobj::index_t idx_3 = shapes[s].mesh.indices[index_offset + 2];
